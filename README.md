...
一个运行在 ChatGPT 对话内、移动端优先的私人双人共读 MCP App。阅读者是小辞，AI 伴侣是 Elias。小辞可以导入自己的小说文本或漫画图片，记录两人的阅读位置，并使用补课同步、轻量短评、书签、摘录与短评 Dock。
...
- 小辞与 Elias 的正文批注支持精确字符范围、幂等写入和 source hash / segmentation version 防错位。
- 英文公版文学入口使用独立 provider；当前只导入明确标记为公共领域且没有译者的英文原文。
...
- D1 保存 session、位置、偏好、批注和 source metadata 等结构化状态；小说正文与漫画图片存放在私有 R2，本地 IndexedDB 仅作设备缓存。
...
- 公版书库元数据来自 Gutendex / Project Gutenberg。版权标记会被双重过滤，但使用者仍应按所在地区法律确认作品状态；应用不导入版权状态未知或带译者的版本。
